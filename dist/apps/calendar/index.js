...
import { CreateEventInputSchema, ListEventsInputSchema, FindFreeTimeInputSchema, UpdateEventInputSchema, DeleteEventInputSchema, } from './schemas.js';
import { generateCreateEventScript, generateListEventsScript, generateFindFreeTimeScript, generateUpdateEventScript, generateDeleteEventScript, } from './scripts.js';
...
function parseFreeTimeOutput(output) {
    const lines = output.trim().split('\\n').filter(Boolean);
    const slots = [];
    for (const line of lines) {
        const parts = line.split('||');
...
export async function handleCalendarTool(request) {
    ...
    return [
        {
            ...
            description: 'Create a new event in Apple Calendar. Supports all-day and timed events with optional location, notes, attendees, and alerts.',
            inputSchema: {
                type: 'object',
                properties: {
                    title: {
        ...
        },
        {
            ...
            description: 'List calendar events within a date range. Can filter by specific calendar. Returns up to 100 events.',
            inputSchema: {
                type: 'object',
                properties: {
                    startDate: {
        ...
        },
        {
            ...
            description: 'Find available time slots in your calendar for scheduling meetings. Analyzes existing events and returns free gaps.',
            inputSchema: {
                type: 'object',
                properties: {
                    date: {
        ...
        },
        {
            ...
            description: 'Update an existing calendar event. Can modify title, dates, location, or notes.',
            inputSchema: {
                type: 'object',
                properties: {
                    eventId: {
        ...
        },
        {
            ...
            description: 'Delete a calendar event. Can find by event ID (most reliable) or by title with optional date filter.',
            inputSchema: {
                type: 'object',
                properties: {
                    eventId: {
